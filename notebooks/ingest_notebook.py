...
def _maybe_fail() -> None:
    ...
    if scenario == "schema_drift":
        # Handle schema drift by adding default event_ts
        pass
...
def run_pipeline(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Handle schema drift by adding missing event_ts column
    if scenario == "schema_drift":
        for row in rows:
            if "event_ts" not in row or row.get("event_ts") is None:
                row["event_ts"] = datetime.utcnow().isoformat()
