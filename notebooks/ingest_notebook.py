...
def _maybe_fail() -> None:
    ...
    if scenario == "schema_drift":
        # Handle schema drift gracefully - allow missing optional columns
        pass
...
def validate_row(row: Dict[str, Any], lenient: bool = False) -> bool:
    for key in EXPECTED_SCHEMA:
        # In lenient mode, skip validation for missing columns
        if lenient and key not in row:
            continue
...
def run_pipeline(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    _maybe_fail()
    scenario = _get_param("scenario", "").strip().lower()
    lenient = scenario == "schema_drift"
    good, bad = [], []
    for r in rows:
        if validate_row(r, lenient=lenient):
            good.append(r)
        else:
            bad.append(r)
