...
def check_quality(count: int, message: str, threshold: float = 0.1) -> None:
    ...
    if count > 0:
        if ratio > threshold:
            print(f"WARNING: {message} (rows={count}, ratio={ratio:.2%}) - EXCEEDS THRESHOLD")
        else:
            print(f"INFO: {message} (rows={count}, ratio={ratio:.2%}) - within acceptable limits")

# COMMAND ----------

# Run quality checks for retail_orders pipeline
if pipeline_id == "retail_orders":
    # Check for null timestamps - handle gracefully instead of failing
